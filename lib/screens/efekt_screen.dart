...
class EfektScreen extends StatelessWidget {
  ...
  // 16 figür — id 0-15, hepsi firmware'de gerçekten çalışır
  static const _figurler = [
    ...
    {'id': 3,  'ad': 'Kar\nTanesi', 'ikon': Icons.ac_unit,                 'renk': 0xFFADD8E6},
    {'id': 4,  'ad': 'Spiral',      'ikon': Icons.cyclone,                 'renk': 0xFF00E5FF},
    {'id': 5,  'ad': 'Sağa →',      'ikon': Icons.arrow_forward,           'renk': 0xFF4CAF50},
    {'id': 6,  'ad': '← Sola',      'ikon': Icons.arrow_back,              'renk': 0xFF00BCD4},
    {'id': 7,  'ad': '↑ Yukarı',    'ikon': Icons.arrow_upward,            'renk': 0xFFFF9800},
    {'id': 8,  'ad': '↓ Aşağı',     'ikon': Icons.arrow_downward,          'renk': 0xFFE91E63},
    {'id': 9,  'ad': 'Hedef',       'ikon': Icons.gps_fixed,               'renk': 0xFFFF5722},
    {'id': 10, 'ad': 'Şimşek',      'ikon': Icons.bolt,                    'renk': 0xFFFFFF00},
    {'id': 11, 'ad': 'Elmas',       'ikon': Icons.diamond,                 'renk': 0xFF7B2FBE},
    {'id': 12, 'ad': 'Gülen\nYüz',  'ikon': Icons.sentiment_satisfied,     'renk': 0xFFFFEB3B},
    {'id': 13, 'ad': 'Kelebek',     'ikon': Icons.blur_on,                 'renk': 0xFFE91E63},
    {'id': 14, 'ad': '↔ Çift Ok',   'ikon': Icons.swap_horiz,              'renk': 0xFF00E5FF},
    {'id': 15, 'ad': '↺ Döngü',     'ikon': Icons.rotate_right,            'renk': 0xFF4CAF50},
  ];
  ...
  Widget build(BuildContext context) {
    ...
    final figSize   = ((s['figureSize']  ?? 3) as num).toDouble();
    final fxSpeed   = ((s['fxSpeed']    ?? 50) as num).toDouble();
    ...
    return ListView(padding: const EdgeInsets.all(14), children: [
      ...
      _izgara(_yaziAnimler, yaziAnim, (id) => svc.send({'textAnim': id}), crossCount: 3),
      const SizedBox(height: 18),

      // ── Çerçeve ─────────────────────────────────────────────
      _baslik('🖼️ Çerçeve Stili'),
      const SizedBox(height: 8),
      _izgara(_cerceveler, cerceve, (id) => svc.send({'borderAnim': id}), crossCount: 4),
      if (cerceve > 0) ...[
      ...
      const SizedBox(height: 8),
      _izgara(_arkaPlanlar, arka, (id) => svc.send({'bgFill': id}), crossCount: 4),
      const SizedBox(height: 18),

      // ── Özel Figürler ────────────────────────────────────────
      _baslik('✦ Özel Figürler'),
      const SizedBox(height: 4),
      const Text('16 figür — 4 ok yönü dahil, hepsi animasyonlu',
        style: TextStyle(fontSize: 11, color: Colors.grey)),
      const SizedBox(height: 8),
      _izgara(_figurler, figur, (id) => svc.send({'figureMode': id}), crossCount: 4),
      if (figur > 0) ...[
        const SizedBox(height: 10),
        _ayarKart('Figür Ayarları', [
          _sliderSatir(Icons.zoom_out_map, 'Boyut', figSize, 1, 8,
            Colors.pink, (v) => svc.send({'figureSize': v.round()})),
          const SizedBox(height: 6),
          _sliderSatir(Icons.speed, 'Animasyon Hızı', fxSpeed, 1, 100,
            const Color(0xFF00E5FF), (v) => svc.send({'fxSpeed': v.round()})),
        ]),
      ],
  ...
  Widget _izgara(List<Map<String,dynamic>> items, int secili,
      Function(int) tiklandi, {int crossCount = 4}) {
        ...
        return GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(
                color: aktif ? renk : const Color(0xFF2A2A3E), width: aktif ? 2 : 1),
              boxShadow: aktif
                ? [BoxShadow(color: renk.withOpacity(0.3), blurRadius: 8, spreadRadius: 1)]
                : [],
            ),
            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(item['ikon'] as IconData, color: aktif ? renk : Colors.grey, size: 22),
              const SizedBox(height: 4),
              Text(item['ad'] as String,
                style: TextStyle(fontSize: 9, color: aktif ? renk : Colors.grey),
  ...
  Widget _ayarKart(String baslik, List<Widget> children) => Container(
    ...
    decoration: BoxDecoration(
      color: const Color(0xFF12121F), borderRadius: BorderRadius.circular(10)),
  ...
  Widget _sliderSatir(IconData ikon, String etiket,
      double deger, double min, double max, Color renk, ValueChanged<double> degisti) {
    return Row(children: [
      Icon(ikon, color: renk, size: 17),
      const SizedBox(width: 6),
      SizedBox(width: 70, child: Text(etiket, style: const TextStyle(fontSize: 11))),
      Expanded(child: SliderTheme(
        data: SliderThemeData(activeTrackColor: renk, thumbColor: renk,
...
Color _hsvColor(double hue) => HSVColor.fromAHSV(1, hue / 255 * 360, 1, 1).toColor();
